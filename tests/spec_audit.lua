	...
	return {
		{
			...
			function()
				...
				local s = assert(compare.open({ path = fixture.file, base = "HEAD" }))
				wait_for(function()
		...
		},
		{
			...
			function()
				...
				local compare = require("lazyvcs.compare")
				local session = assert(compare.open({ base = "HEAD" }))
				local ok, err = pcall(function()
					wait_for(function()
		...
		},
		{
			...
			function()
				...
				local compare = require("lazyvcs.compare")
				local session = assert(compare.open({ path = fixture.root, base = "HEAD" }))
				wait_for(function()
